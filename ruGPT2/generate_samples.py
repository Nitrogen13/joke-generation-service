...
def get_token_stream(model, context_tokens, tokenizer, args):
    ...
    batch_token_iterator = sample_sequence_batch(model, context_tokens_tensor, context_length_tensor, attention_mask,
                                                 position_ids, tokenizer, args)
...
def sample_sequence_batch(model, context_tokens, context_lengths, attention_mask, position_ids, tokenizer, args,
                          maxlen=None):
    model.eval()
    with torch.no_grad():
        ...
        while context_length <= (maxlen):
            ...
            if args.recompute:
                ...
                logits, layer_past = model(tokens2use, positions2use, attention_mask, layer_past=layer_past,
                                           get_present=True)
